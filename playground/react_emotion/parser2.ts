...
function ImportSpecifiers(parent, specifiers) {
  ...
  for (const specifier of specifiers) {
    if (specifier.type === 'ImportNamespaceSpecifier') {
      // import * as React from 'react'
      importSpecifiers[specifier.local.name] = {
  ...
  if (node.type === 'ImportDeclaration') {
    ...
    return {
      removeSpec: function (spec) {
        delete this.specifiers[spec];
        if (Object.keys(this.specifiers).length === 0) {
    ...
    if (
      ...
      // should specifiers be set to:
      // { name: 'default', local: 'bar'} ??
      return {
